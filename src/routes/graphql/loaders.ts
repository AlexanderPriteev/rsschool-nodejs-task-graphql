...
const membersLoader = (prisma: PrismaClient) => new DataLoader(async (keys) => {
...
const profileLoader = (prisma: PrismaClient) => new DataLoader(async (keys) => {
...
const postLoader = (prisma: PrismaClient) => new DataLoader(async (keys) => {
...
const userSubscribedToLoader = (prisma: PrismaClient) => new DataLoader(async (keys) => {
...
const subscribedToUserLoader = (prisma: PrismaClient) => new DataLoader(async (keys) => {
    const subscriptions = await prisma.user.findMany({
...
export const getLoader = <T>(id: string | string[], context: IContext, loaderCreator: string): DataLoader<string| string[], T> | null => {
